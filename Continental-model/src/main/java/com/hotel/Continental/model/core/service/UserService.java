...
public class UserService implements IUserService {
    ...
    public EntityResult userInsert(Map<?, ?> attrMap) {
        ...
        if (!attrMap.containsKey(UserDao.USER) || !attrMap.containsKey(UserDao.PASSWORD) || !attrMap.containsKey(UserDao.NAME) ||
                !attrMap.containsKey(UserDao.SURNAME)|| !attrMap.containsKey(UserDao.NIF)||!attrMap.containsKey(UserDao.COUNTRY_CODE)) {
        ...
        if(attrMap.containsKey(RoleDao.ID_ROLENAME)) {
            ...
            attrRole.put(UserRoleDao.USER, attrMap.get(UserDao.USER));
    ...
    private EntityResult checkUpdate(Map<String, Object> attrMap) {
        ...
        if (attrMap.get(UserDao.COUNTRY_CODE) != null) {
            attrMap.put(UserDao.COUNTRY_CODE, ((String) attrMap.remove(UserDao.COUNTRY_CODE)).toUpperCase());
            //Si el country code no mide 2 Caracteres esta mal
            if (((String) attrMap.get(UserDao.COUNTRY_CODE)).length() != 2) {
                EntityResult er = new EntityResultMapImpl();
            ...
            if (!checkCountryCode(((String) attrMap.get(UserDao.COUNTRY_CODE)))) {
                EntityResult er = new EntityResultMapImpl();
                er.setCode(EntityResult.OPERATION_WRONG);
        ...
        if (attrMap.get(UserDao.NIF) != null) {
            ...
            if (!checkDocument((String) attrMap.get(UserDao.NIF), (String) attrMap.get(UserDao.COUNTRY_CODE))) {
                EntityResult er = new EntityResultMapImpl();
                er.setCode(EntityResult.OPERATION_WRONG);
            ...
            if (existsKeymap(this.userDao,Collections.singletonMap(UserDao.NIF, attrMap.get(UserDao.NIF)),List.of(UserDao.NIF))) {
                EntityResult er = new EntityResultMapImpl();
                er.setCode(EntityResult.OPERATION_WRONG);
                er.setMessage(Messages.DOCUMENT_ALREADY_EXIST);
                return er;
            }
            if (existsKeymap(this.userDao,Collections.singletonMap(UserDao.USER, attrMap.get(UserDao.USER)),List.of(UserDao.USER))) {
                EntityResult er = new EntityResultMapImpl();
                er.setCode(EntityResult.OPERATION_WRONG);
